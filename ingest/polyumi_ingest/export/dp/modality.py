...
``export-dp`` emits the visuomotor keys and nothing else. ``export-polyumi`` emits those plus
whatever modalities it is configured with — audio today, the finger camera later. Both run the
*same* ``_export_episode``: the segmentation, chirp trim, pose resolution and provenance are hard
enough to get right once, and a second copy would drift.
...
sidecar, which is what lets ``export-dp`` keep its contract unchanged.
"""
