...
| Latency compensation, gopro + proprio | **done**, matches UMI's scheme, unit-tested. Current values live in [inference.yaml](../ros2_ws/src/polyumi_ros2/config/inference.yaml); how to re-measure them is in [calibration-instructions.md](calibration-instructions.md) |
...
| | |
|---|---|
| `PoseTrajectoryInterpolator` port | 10 gtests, expected values generated by running UMI's own Python on the same inputs; two more pin what our finite speed limits do that UMI's infinite ones cannot |
| Cartesian impedance law | 13 gtests — zero-error/zero-torque, force opposes error, clip saturates, nullspace ⊥ task, rate bound |
| `colcon test` on the NUC | Release build clean |
