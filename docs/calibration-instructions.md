...
- **A failed `SetLoad` aborts bringup**, rather than warning and carrying on. `ros2 service call` exits 0 even when the body says `success: false`, so bringup pipes it through `grep -q success=True` and refuses to spawn `fr3_arm_controller` if that fails. The spawner is the point of no return — after it the robot is in `Move` mode and the payload cannot be set again without deactivating everything — and the only other symptom is TCP droop, which you have to be watching for. If bringup dies here, read the `/service_server` log before re-launching.
