...
#   POLICY=vista DATASET=... ./train_policy.sh --model vista_vt --dry-run
#
# POLICY resolves config/policy.<name>.env via config/policy_select.sh, which names the fork
# directory, image tag, container entrypoint and default Hydra workspace config. CONFIG_NAME
# overrides that last one for a single run; it is forwarded as an env var rather than a
# passthrough override because Hydra cannot set --config-name that way, and both forks'
# docker/train.sh read it.
set -euo pipefail
...
# Sets IMAGE, POLICY_DIR, CONFIG_NAME and TRAIN_CMD from config/policy.${POLICY}.env.
...
# the container ends up running as under rootless. /data/dataset.zarr.zip is the path every fork's
# entrypoint defaults to, so no dataset override has to be forwarded; outputs land in the
# bind-mounted dir.
